...
class Jobs extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if($this->input->get('mobile')) {
                $selection = array('jobs.cus_number' => $this->input->get('mobile'));
        }
        if ($this->input->post()) {
            //$orderby = 'jobs.id desc';
            $selection = null;
            ...
            if($this->input->post('filter3')) {
                SearchJobs($this->input->post('filter3'));
            }
            if($this->input->post('filter4')) {
                $selection = array('DATE(update_date)' => $this->input->post('filter4'));
            }
            
            
            $this->data['filter1'] = $this->input->post('filter');
            $this->data['filter2'] = $this->input->post('filter2');
            $this->data['filter3'] = $this->input->post('filter3');
            $this->data['value1'] = $this->input->post('value');
            $this->data['record'] = join_select('jobs.*,tech.first_name,tech.last_name','jobs', 'technician', 'tech', 'id', $selection, null, null, null, $orderby);
            //echo $this->db->last_query();exit;
        } else{
            $this->data['record'] = join_select('jobs.*,tech.first_name,tech.last_name','jobs', 'technician', 'tech', 'id', $selection, null, null, $orderby);
            //echo $this->db->last_query();exit;
        }       
        //print_r($_GET);exit;
    ...
    function customers()
    {
        ...
        if ($this->input->post()) {
            ...
            $this->data['filter3'] = $this->input->post('filter3');
            $this->data['value1'] = $this->input->post('value');
            $this->data['record'] = join_select('jobs.*,tech.first_name,tech.last_name','jobs', 'created_by', 'tech', 'id', $selection, null, null, $group_by, $orderby);
            //echo $this->db->last_query();exit;
        } else
            $this->data['record'] = join_select('jobs.*,tech.first_name,tech.last_name','jobs', 'created_by', 'tech', 'id', null, null, $group_by, $orderby);
            //echo $this->db->last_query();exit;
        $this->data['page_title'] = 'BudgetMobile :: Job Listing';
        $this->load->view('customers_listing', $this->data);
    }
    ...
    public function create($type = null)
    {
        ...
        if ($this->input->post()) {
            $postData = array(
                ...
                'cus_business_name' => $this->input->post('cus_business_name'),
            ...
            $this->data['title'] = $type == 'mob' ? 'Mobile Phone Repair Job' :
                'Laptop Repair Job';
            $this->data['page_title'] = $type == 'mob' ? 'BudgetMobile :: Create Mobile Job' :
                'BudgetMobile :: Create Laptop Job';
            ;
    ...
    {
        $this->data['record'] = join_select('jobs.*,users.first_name,users.last_name',
            'jobs', 'created_by', 'users', 'id', array('job_slug' => $job_slug));
        $this->data['tech'] = generic_select('tech', array('status' => 1));
        $this->data['job'] = $this->data['record'][0];
        $this->data['job_type'] = $this->data['job']->job_type;
        $this->data['title'] = $this->data['job']->job_type == 'mob' ? 'Mobile Phone Repair Job' : 'Laptop Repair Job';
    ...
    {
        if ($this->input->post()) {
            if (md5($this->input->post('old_password')) === getSingle('users', 'password',
    ...
    {
        $this->data['record'] = join_select('jobs.*,users.first_name,users.last_name',
            'jobs', 'created_by', 'users', 'id', array('job_slug' => $_POST['job_id']));
        $this->data['job'] = $this->data['record'][0];
        $this->data['job_type'] = $this->data['job']->job_type;
        $this->data['title'] = $this->data['job']->job_type == 'mob' ? 'Mobile Detail' :
            'Laptop Detail';
        $temp = generic_select_row('tech', array('id' => $this->data['job']->technician));
        if (is_array($temp) || is_object($temp))
            $this->data['tech'] = $temp->first_name . ' ' . $temp->last_name;
        ...
        if ($this->data['job'] && strlen(trim($this->data['job']->cus_email)) > 2) {
            ...
            $this->email->from($from_email, 'Budget Mobiles');
            $this->email->to($to_email);
            $this->email->subject(mb_strtoupper($this->data['job']->job_slug) . ' ' . $this->
                data['job']->status);
    ...
    function get_customer_info()
    {
        $json = array();
        $cus_number        = $this->input->post('cus_number');
        $json['user_info'] = $this->generic_model->selectwhere('jobs', 'cus_number', $cus_number);
        //echo $this->db->last_query();
        if(!empty($json['user_info']))
        {
            $json['status'] = 1;
        }else{
            $json['status'] = 0;
